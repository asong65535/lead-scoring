...
class TestEngagementVelocity7d:
    def test_accelerating(self, lead):
        ...
        assert engagement_velocity_7d(lead, _bucket(events), AS_OF) == 2
    ...
    def test_email_unsubscribe_subtracts_one(self, lead):
        ...
        assert engagement_velocity_7d(lead, _bucket(events), AS_OF) == -1
    ...
    def test_no_events_in_window(self, lead):
        ...
        assert engagement_velocity_7d(lead, _bucket([outside]), AS_OF) == 0

    def test_empty_events(self, lead):
        assert engagement_velocity_7d(lead, {"_all": []}, AS_OF) == 0
...
class TestIsEngagementIncreasing:
    def test_true_when_velocity_positive(self, lead):
        ...
        assert is_engagement_increasing(lead, _bucket(events), AS_OF) is True

    def test_false_when_velocity_zero(self, lead):
        events = [_recent_event(), _older_event()]
        assert is_engagement_increasing(lead, _bucket(events), AS_OF) is False
